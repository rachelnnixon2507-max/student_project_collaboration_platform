...
public class AdminSeeder {
    ...
    CommandLineRunner seedSampleData() {
        return args -> {
            // Seed initial sample students & faculty if database has zero users
            if (userRepository.count() == 0) {
                ...
                // Announcements
                announcementRepository.save(Announcement.builder().title("Project Evaluation Week Announced").content("Faculty evaluations will open next Monday. All teams must submit progress reports.").scope(AnnouncementScope.ALL).createdBy(s1.getId()).build());
